...
```{r}
res.cox <- coxph(Surv(nctdel, fail) ~ male + black + hisp + count_sn, data =  kelly)
...
```{r}
# fake data
times <- 1 + c(rgeom(20, .3), rgeom(20,.05))
...
kernel.transformation <- function(time, censored, total_bins, kernels, i) {
  ...
  X <- array(kernel.weights[i:time+i,],dim=c(time,kernels)) ###how to translate this line??
...
for(i in 2:length(kelly$nctdel)) {
  d2 <- rbind(d2, kernel.transformation(kelly$nctdel[i], as.logical(kelly$fail[i]), nrows(kelly), ncol(kelly)-1, i))
}
