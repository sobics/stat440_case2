...
##DO WE INCLUDE THIS THING BELOW??
...
##Kernel Regression with 26 Bins
```{r}
# set up the kernels
bins <- cut(kelly$nctdel, c(-Inf,seq(0,2.05,0.1),10,13,15,17,Inf))
#bins <- cut(kelly$nctdel, c(-Inf,0,0.5, 0.7, 1.3, 2.05,10,15,Inf)) #manually choose bins based on quantiles and my own knowledge
levels(bins) <- as.character(seq(1:nlevels(bins)))
kelly$bins = bins
...
d2 <- kernel.transformation(as.numeric(bins)[1], as.logical(kelly$fail[1]))
...
X.pred <- array(as.numeric(kernel.weights[,-1]),dim=c(nlevels(bins),ncol(kernel.weights)-1))

hazard.logodds <- predict(m2, newdata = data.frame(X.pred))
hazard <- exp(hazard.logodds)/(1+exp(hazard.logodds))
plot(1:nlevels(bins), hazard, type='l')
...
#this doesn't work
```{r include = F}
hoslem.test(kelly$fail, round(fitted(m2)))
```

```{r}
X.test <- array(as.matrix(kelly %>% select(nctdel, male, black, hisp, count_sn)), dim = c(nrow(kelly), ncol(kernel.weights)-1))

kern.pred <- round(predict.glm(m2, newdata = data.frame(X.test), type = 'response'))
mean(kern.pred != kelly$fail)
```

Overall, the model fits the true dataset 82% of the time, which reaffirms the goodness-of-fit of the model.
