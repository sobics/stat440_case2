...
ggplot(kelly, aes(nctdel, group = black)) + geom_line()
...
##Gender Bias
# Side by side histograms of wait times by gender
```{r}
ggplot(kelly, aes(x=nctdel)) +
  facet_wrap(~ male) +
  geom_histogram(aes(y=..count../sum(..count..))) +
...
In an effort to understand factors predictive of wait time, we could model the wait time for each patient as a Poisson distribution. However, after attempting to fit a Poisson distribution with $\lambda = 1.46$, we noticed that it did not capture the data robustly enough. 
```{r}
...
```
Below are three potential approaches to alleviate this challenge:

1. We plan to separate the model into bins, and to model each bin with a piecewise constant hazard model.
