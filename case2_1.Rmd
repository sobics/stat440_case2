...
We could model the wait time for each patient as a Poisson distribution. However, after attempting to fit a Poisson distribution with $\lambda = 1.46$, we notice that it does not capture the data robustly enough. Below are two potential approaches to alleviate this challenge:
...
3. We could also cluster the data to check to see if people with similar characters have similar wait times, or if there is not a distinction based on the covariates.
